...
let NOTIFS=[{id:1,type:'system',cls:'b',title:'Welcome to UTHM PKU',body:'Your account is active. Register for a queue or book an appointment to get started.',time:'Just now',read:false,targetId:null}];
  ...
  if(window.innerWidth<=640){
    ...
    if(op){
      ...
      if(!tc){
        ...
        tc.style.cssText='position:fixed;top:0;right:0;bottom:0;left:var(--sw);background:rgba(0,0,0,.45);z-index:299;opacity:0;transition:opacity .28s cubic-bezier(.4,0,.2,1);';
        requestAnimationFrame(()=>requestAnimationFrame(()=>tc.style.opacity='1'));
      ...
      const tc=document.getElementById('mobTapCatcher');
      if(tc){tc.style.opacity='0';setTimeout(()=>tc.remove(),280);}
    }
  }else{
    S.sbOpen=!S.sbOpen;
...
function addNotif(type,cls,title,body,targetId){const tid=targetId||(S.user?S.user.id:null);NOTIFS.unshift({id:NC++,type,cls,title,body,time:fmtTime(),read:false,targetId:tid});updateNdot();}
function myNotifs(){const u=S.user;if(!u)return[];return u.role==='admin'?NOTIFS:NOTIFS.filter(n=>!n.targetId||n.targetId===u.id);}
function updateNdot(){const n=myNotifs().filter(x=>!x.read).length;const d=document.getElementById('ndot');if(d)d.style.display=n?'block':'none';}
...
function markAllRead(){myNotifs().forEach(n=>n.read=true);buildNP();updateNdot();const el=document.getElementById('notifPgList');if(el)el.innerHTML=renderNotifCards(myNotifs());toast('All marked as read','s');}
...
function doLogin(){
  ...
  if(ud&&ud._profilePic)S.profilePic=ud._profilePic;else S.profilePic=null;
...
function doRegister(){
  ...
  addNotif('system','b','New Registration',`${newU.name} (${id}) registered as ${role}. Awaiting approval.`,'ADM001');
...
function navItems(){
  ...
  if(r==='pharmacist')return[{s:'Main'},{v:'dashboard',ic:'dash',lbl:'Dashboard'},{v:'pharma-mgmt',ic:'phar',lbl:'Prescription Queue',bdg:()=>PRESCRIPTIONS.filter(p=>p.status==='pending'||p.status==='processing').length||''},{s:'Inventory'},{v:'Medicine-inventory',ic:'pkg',lbl:'Medicine Inventory'},{s:'Account'},{v:'profile',ic:'user',lbl:'Profile'}];
  return[{s:'Main'},{v:'dashboard',ic:'dash',lbl:'Dashboard'},{v:'patients',ic:'user',lbl:'Patient Management'},{v:'queue-mgmt',ic:'qr',lbl:'Queue Management',bdg:()=>QUEUE.filter(q=>q.status==='waiting').length||''},{v:'appointments',ic:'cal',lbl:'All Appointments'},{s:'Analytics'},{v:'analytics',ic:'chart',lbl:'Analytics & Reports'},{v:'billing-admin',ic:'bill',lbl:'Financial Reports'},{s:'System'},{v:'staff-mgmt',ic:'staff2',lbl:'Staff Management'},{v:'approvals',ic:'user',lbl:'Pending Approvals',bdg:()=>PENDING.filter(u=>!u.approved).length||''},{v:'profile-reqs',ic:'user',lbl:'Profile Edit Requests',bdg:()=>PROFILE_REQS.filter(r=>r.status==='pending').length||''},{v:'Medicine-inventory',ic:'pkg',lbl:'Medicine Inventory'},{v:'settings',ic:'sett',lbl:'System Settings'},{s:'Account'},{v:'profile',ic:'user',lbl:'Admin Profile'}];
}
const PT={dashboard:'Dashboard',appointments:'Appointments',queue:'Walk In','queue-mgmt':'Queue Management',billing:'Billing & Payment','billing-admin':'Financial Reports',pharmacy:'Pharmacy','pharma-mgmt':'Prescription Queue',profile:'Profile',patients:'Patient Management',analytics:'Analytics & Reports',settings:'System Settings',schedule:'My Schedule',records:'Patient Records','Medicine-inventory':'Medicine Inventory','room-status':'Room Status','staff-mgmt':'Staff Management','med-history':'Medical History',notifications:'Notifications',feedback:'Feedback & Help',approvals:'Pending Approvals','profile-reqs':'Profile Edit Requests'};
...
function callPatient(id){const q=QUEUE.find(x=>x.id===id);if(!q)return;QCALLED=parseInt(q.number.split('-')[1]);q.status='consulting';addNotif('queue','g','Your Turn — '+q.number,'Proceed to consultation room. Doctor: '+q.doctor,q.pid);toast(q.number+' called — '+q.patient,'s');buildNav();nav('queue-mgmt');}
function skipPatient(id){const q=QUEUE.find(x=>x.id===id);if(!q)return;q.status='skipped';addNotif('queue','o','Queue Skipped','Number '+q.number+' skipped. Please report to front desk.',q.pid);toast(q.number+' skipped','w');nav('queue-mgmt');}
function donePatient(id){const q=QUEUE.find(x=>x.id===id);if(!q)return;q.status='done';const billId='INV-'+new Date().getFullYear()+'-'+String(BC++).padStart(4,'0');BILLS.push({id:billId,pid:q.pid,patient:q.patient,items:[{n:'Consultation Fee',p:q.dept==='Dental'?SYS.dentalFee:SYS.consultFee}],status:'unpaid',method:null,date:fmtDate()});addNotif('billing','o','Invoice Generated','Invoice '+billId+' for your visit. Pay at billing counter.',q.pid);if(q.dept==='GP'){const rxId='RX-'+new Date().getFullYear()+'-'+String(RC++).padStart(4,'0');PRESCRIPTIONS.push({id:'PR'+Date.now(),rx:rxId,pid:q.pid,patient:q.patient,doctor:q.doctor,meds:'As prescribed by doctor',status:'pending',counter:'—',date:fmtDate()});addNotif('pharma','','Prescription Issued','Prescription '+rxId+' issued. Collect at pharmacy.',q.pid);}toast('Done — bill and prescription created','s');nav('queue-mgmt');}
...
function cancelAppt(id,btn){const a=APPTS.find(x=>x.id===id);if(!a)return;a.status='cancelled';const row=btn?.closest('tr');if(row){const b=row.querySelector('.badge');if(b){b.className='badge bd';b.textContent='cancelled';}btn.remove();}addNotif('appt','o','Appointment Cancelled','Appointment '+a.id+' cancelled.',a.pid);toast('Cancelled','i');}
...
function processPayment(id){const b=BILLS.find(x=>x.id===id);if(!b)return;b.status='paid';b.method=S.payMethod==='duitnow'?'DuitNow':S.payMethod==='smap'?'SMAP':'Cash';addNotif('billing','g','Payment Confirmed','Invoice '+b.id+' paid via '+b.method+'.',b.pid);closeModal();dlReceipt(id);nav('billing');}
...
function vMedicineInventory(){document.getElementById('vc').innerHTML='<div class="ph-row"><div class="ph" style="margin-bottom:0"><h1>Medicine Inventory</h1></div><button class="btn btn-primary btn-sm" onclick="openReorder(-1)">+ Reorder</button></div><div style="height:1.1rem"></div>'+sgHTML([{ic:svgi(P.rx),val:MedicineS.length,lbl:'Medicine Lines'},{cls:'g',ic:svgi(P.chk,'#27ae60'),val:MedicineS.filter(d=>d.stock>d.min*1.5).length,lbl:'Adequate'},{cls:'o',ic:svgi(P.clk,'#f39c12'),val:MedicineS.filter(d=>d.stock<=d.min*1.5).length,lbl:'Low Stock'},{cls:'b',ic:svgi(P.users,'#2980b9'),val:MedicineS.reduce((s,d)=>s+d.stock,0),lbl:'Total Units'}])+(MedicineS.some(d=>d.stock<=d.min*1.5)?'<div class="ib ib-o">'+IC.warn+'<span><strong>Low stock:</strong> '+MedicineS.filter(d=>d.stock<=d.min*1.5).map(d=>d.name).join(', ')+'</span></div>':'')+'<div class="card"><div class="card-bd"><div class="tw"><table><thead><tr><th>Medication</th><th>Stock</th><th>Min</th><th>Unit</th><th>Status</th><th>Level</th><th>Action</th></tr></thead><tbody>'+MedicineS.map((d,i)=>{const c=d.stock<=d.min,l=d.stock<=d.min*1.5,p=Math.min(Math.round(d.stock/500*100),100),clr=c?'var(--danger)':l?'var(--orange)':'var(--green)';return'<tr><td><strong>'+d.name+'</strong></td><td>'+d.stock+'</td><td>'+d.min+'</td><td>'+d.unit+'</td><td><span class="badge '+(c?'bd':l?'bw':'bs')+'">'+(c?'Critical':l?'Low':'OK')+'</span></td><td style="min-width:95px"><div class="prog-bar"><div class="prog-fill" style="width:'+p+'%;background:'+clr+'"></div></div></td><td><button class="btn btn-ghost btn-sm" onclick="openReorder('+i+')">Reorder</button></td></tr>';}).join('')+'</tbody></table></div></div></div>';}
function openReorder(idx){openModal('Reorder Request','<div class="fg"><label class="lbl">Medication</label><select class="inp" id="rrMedicine">'+MedicineS.map((d,i)=>'<option value="'+i+'" '+(i===idx?'selected':'')+'>'+d.name+' (Stock: '+d.stock+')</option>').join('')+'</select></div><div class="r2"><div class="fg"><label class="lbl">Quantity to Add</label><input class="inp" type="number" id="rrQty" value="200" min="1"></div><div class="fg"><label class="lbl">Priority</label><select class="inp"><option>Normal</option><option>Urgent</option><option>Critical</option></select></div></div><div class="fg"><label class="lbl">Reason</label><textarea class="inp" rows="2">Stock approaching minimum threshold.</textarea></div>',[{lbl:'Cancel',cls:'btn-ghost',fn:'closeModal()'},{lbl:'Submit',cls:'btn-primary',fn:'submitReorder()'}]);}
function submitReorder(){const i=parseInt(document.getElementById('rrMedicine')?.value),q=parseInt(document.getElementById('rrQty')?.value||0);if(isNaN(q)||q<=0){toast('Enter valid quantity','e');return;}if(!isNaN(i)&&MedicineS[i])MedicineS[i].stock+=q;closeModal();toast('Reorder submitted & stock updated','s');nav('Medicine-inventory');}
...
function submitProfileEdit(){PROFILE_REQS.push({id:'PER-'+Date.now(),pid:S.user.id,patient:S.user.name,type:'profile_info',changes:{phone:document.getElementById('epPh')?.value,blood:document.getElementById('epBl')?.value,allergy:document.getElementById('epAl')?.value},status:'pending',date:fmtDate()});addNotif('system','b','Profile Edit Request','Profile update submitted. Awaiting admin approval.',S.user.id);closeModal();buildNav();toast('Edit request submitted for approval','s');}
...
function markDone(id){const a=APPTS.find(x=>x.id===id);if(!a)return;a.status='completed';MED_RECORDS.push({id:'MR-'+new Date().getFullYear()+'-'+String(MRC++).padStart(4,'0'),pid:a.pid,patient:a.patient,dept:a.dept,doctor:a.doctor,complaint:'Consultation',diagnosis:'As documented by doctor',treatment:'As prescribed',date:a.date,fu:'As needed'});addNotif('appt','g','Consultation Complete','Consultation with '+a.doctor+' complete.',a.pid);toast('Appointment completed','s');nav('schedule');}
...
function approveUser(id){const u=PENDING.find(x=>x.id===id);if(!u)return;u.approved=true;USERS_DB[id]={...u,role:'user',blood:'—',allergy:'None',approved:true};addNotif('system','g','Account Approved',u.name+' ('+id+') can now login.',id);buildNav();toast(u.name+' approved!','s');nav('approvals');}
...
function approvePerReq(id){const r=PROFILE_REQS.find(x=>x.id===id);if(!r)return;r.status='approved';const u=USERS_DB[r.pid];if(u&&r.type==='profile_info'&&r.changes)Object.assign(u,r.changes);if(r.type==='profile_picture'&&r.data){if(u)u._profilePic=r.data;if(S.user&&S.user.id===r.pid){S.profilePic=r.data;updateAvatars();}}addNotif('system','g','Profile Update Approved','Your profile update has been approved.',r.pid);toast('Approved','s');nav('profile-reqs');}
function rejectPerReq(id){const r=PROFILE_REQS.find(x=>x.id===id);if(!r)return;r.status='rejected';addNotif('system','o','Profile Update Rejected','Your profile update was rejected.',r.pid);toast('Rejected','w');nav('profile-reqs');}
function vNotifications(){const vis=myNotifs();document.getElementById('vc').innerHTML='<div class="ph-row"><div class="ph" style="margin-bottom:0"><h1>Notifications</h1></div><button class="btn btn-ghost btn-sm" onclick="markAllRead()">Mark all read</button></div><div style="height:1.1rem"></div><div class="tabs"><div class="tab active" onclick="filterNTab(this,\'all\')">All ('+vis.length+')</div><div class="tab" onclick="filterNTab(this,\'queue\')">Queue</div><div class="tab" onclick="filterNTab(this,\'billing\')">Billing</div><div class="tab" onclick="filterNTab(this,\'appt\')">Appointments</div><div class="tab" onclick="filterNTab(this,\'pharma\')">Pharmacy</div><div class="tab" onclick="filterNTab(this,\'system\')">System</div></div><div id="notifPgList" style="display:flex;flex-direction:column;gap:.6rem">'+renderNotifCards(vis)+'</div>';}
function renderNotifCards(list){return list.length?list.map(n=>{const isAdmin=S.user&&S.user.role==='admin';const targetLabel=isAdmin&&n.targetId?(USERS_DB[n.targetId]?'<span style="font-size:.67rem;background:rgba(139,26,26,.08);color:var(--red);border-radius:4px;padding:.1rem .38rem;margin-left:.38rem">→ '+(USERS_DB[n.targetId].name.split(' ')[0])+'</span>':''):'';return'<div class="card" style="border-left:4px solid '+(n.cls==='g'?'var(--green)':n.cls==='o'?'var(--orange)':n.cls==='b'?'var(--blue)':'var(--red)')+';opacity:'+(n.read?.7:1)+';cursor:pointer" onclick="readNPage('+n.id+',this)"><div style="padding:.85rem 1rem;display:flex;align-items:flex-start;gap:.76rem"><div style="width:7px;height:7px;border-radius:50%;background:'+(n.cls==='g'?'var(--green)':n.cls==='o'?'var(--orange)':n.cls==='b'?'var(--blue)':'var(--red)')+';flex-shrink:0;margin-top:4px"></div><div style="flex:1"><div style="font-weight:600;font-size:.86rem;color:var(--g900);margin-bottom:.2rem">'+n.title+(!n.read?'<span style="display:inline-block;width:6px;height:6px;background:var(--red);border-radius:50%;margin-left:4px;vertical-align:middle"></span>':'')+targetLabel+'</div><div style="font-size:.8rem;color:var(--g500);line-height:1.6">'+n.body+'</div><div style="font-size:.68rem;color:var(--g300);margin-top:.3rem">'+n.time+'</div></div>'+(!n.read?'<span class="badge bd">New</span>':'')+'</div></div>';}).join(''):'<div style="text-align:center;padding:2rem;color:var(--g400);font-size:.82rem">No notifications found</div>';}
function filterNTab(el,type){document.querySelectorAll('.tab').forEach(t=>t.classList.remove('active'));el.classList.add('active');const all=myNotifs(),fl=type==='all'?all:all.filter(n=>n.type===type);const el2=document.getElementById('notifPgList');if(el2)el2.innerHTML=renderNotifCards(fl);}
...
// Add price to Medicines
[0.50,1.20,0.80,0.60,5.00,8.00,0.30,1.50].forEach((p,i)=>{if(MedicineS[i])MedicineS[i].price=p;});
...
function checkInFromAppointment(apptId){
  ...
  addNotif('queue','g','Check-In Successful','Visit '+visitId+' created. Queue number will be issued shortly.',appt.pid);
  return visit;
}
...
function generateQueueFromVisit(visitId){
  ...
  addNotif('queue','','Queue Number Issued','Your number is '+qNum+'. Est. wait: '+(waiting*15)+' min.',visit.patientId);
  return entry;
}
...
function submitConsultation(csId,diagData,rxItems){
  ...
  if(rxItems&&rxItems.length>0){
    ...
    rxItems.forEach(item=>{
      ...
      const Medicine=MedicineS.find(d=>d.name===item.name);
      if(Medicine)Medicine.stock=Math.max(0,Medicine.stock-item.qty);
    });
    if(visit)visit.prescriptionId=pxObj.id;
    auditLog('RX_CREATED','Prescription',rxId,rxItems.length+' items');
    addNotif('pharma','','Prescription Sent to Pharmacy','Prescription '+rxId+' is now being prepared. Patient: '+cs.patientName,cs.patientId);
  }
  ...
  if(rxItems&&rxItems.length>0){
    rxItems.forEach(item=>{
      const Medicine=MedicineS.find(d=>d.name===item.name);
      const price=Medicine?(Medicine.price||0)*item.qty:0;
  ...
  addNotif('appt','g','Consultation Complete','Consultation done.'+(rxItems&&rxItems.length>0?' Prescription sent to pharmacy.':'')+' Invoice '+billId+' generated.',cs.patientId);
...
function vConsultationRoom(){
  ...
  if(activeCs){
    ...
    document.getElementById('vc').innerHTML=
      ...
      +'<div style="display:flex;align-items:center;gap:.75rem;margin-bottom:.8rem"><div style="width:42px;height:42px;border-radius:50%;background:var(--red);display:flex;align-items:center;justify-content:center;color:#fff;font-family:\'Montserrat\',serif;font-size:1rem;font-weight:700;flex-shrink:0">'+ini(activeCs.patientName)+'</div><div><div style="font-weight:600">' +activeCs.patientName+'</div><div style="font-size:.73rem;color:var(--g400)">'+activeCs.patientId+' · Blood: '+(patient.blood||'—')+'</div></div></div>'
...
function confirmAddRx(){
  ...
  window._rxItems.push({name:Medicine.name,qty,dosage,unit});
  closeModal();
...
function viewVisitDetail(visitId){
  ...
  openModal('Visit — '+visitId,
    ...
    +'<div style="font-family:\'Montserrat\',serif;font-size:1.05rem;font-weight:700">'+v.patientName+'</div>'
...
function rxNextStep(id){
  ...
  if(next==='ready'){
    ...
    addNotif('pharma','g','Medication Ready','Your prescription '+rx.rx+' is ready for collection at Counter 1. Please proceed to the pharmacy counter.',rx.pid);
    ...
    addNotif('pharma','g','Medicine Collected','Prescription '+rx.rx+' has been collected. Your visit is complete.',rx.pid);
    // Update visit status
